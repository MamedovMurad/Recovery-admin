...
type DataIndex = keyof SliderType;
...
const Sliders: React.FC = () => {
  ...
  const getColumnSearchProps = (dataIndex: DataIndex): ColumnType<SliderType> => ({
  ...
  function updateSlider(item: SliderType) {
    setOpen(true);
    setslider(item)
  }

  const columns: ColumnsType<SliderType> = [
    {
      title: 'Image',
      dataIndex: 'image',
      key: 'image',
      ...
      render: (_: any, record: SliderType) => <Image
        width={90}
    ...
    },
    {
      ...
      dataIndex: 'action',
      key: 'action',
      render: (_: any, record: SliderType) => {
        return (
          <>
            <Space size={'middle'}>
  ...
  async function fethcSlider() {
    const res = await agent.slider.list()
    !res.error && setsliders(res.data)
    handleOk()
  }

  async function deleteSlider(id: number) {
    const res = await agent.slider.delete(id)
  ...
  return <>
    ...
    >
      <SliderAddEdit slider={slider || undefined} callback={fethcSlider} />
    </Modal>
  </>
